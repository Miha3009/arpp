...
def process_file(period, inmcm_climate, era5_climate, inmcm_anomaly_path, inmcm_anomaly_path2, era5_anomaly_path,
    ...
    climate_dir = Path(f'{climate_input_path}/{period_name}')

    files = []
    for folder in climate_dir.iterdir():
        if folder.is_dir() and not folder.name in ['globsnow', 'aice', 'swe']:
            file = folder / f'{period:02d}.nc'
            if file.exists():
                files.append(file)
    climate_datasets = [xr.open_dataset(f, engine='h5netcdf') for f in files]
    for i, ds in enumerate(climate_datasets):
        climate_datasets[i] = ds.rename({"lat": f"lat{i}", "lon": f"lon{i}"})
    climate_combined = xr.merge(climate_datasets, join='outer')
    climate_combined.to_netcdf(f'{output_path}/{variable}/clim/{period:02d}.nc', engine='h5netcdf')
...
def read_climate(variable, period, path):
    ...
    return ds[f'{variable}_mean'].values

def process_ice(month):
    inmcm_climate = read_climate('aice', month, f'{climate_input_path}/month/aice/{month:02d}.nc')
    era5_climate = read_climate('aice', month, f'{climate_input_path}/month/era5_ice/{month:02d}.nc')
