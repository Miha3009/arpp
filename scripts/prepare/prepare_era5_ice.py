...
era5_input_path = "../../data/raw/ice_era5/"
era5_output_path = "../../data/prepare/era5_ice/"
...
def open_netcdf4(file):
    ...
    vals = ds.siconc.to_numpy()
    ds.close()
    return valid_time, vals

def process_file(date):
    date_str = date.strftime('%Y%m%d')
    filename = f"era5_ice_cover_{date_str[:4]}_{date_str[4:6]}.nc"
    ...
    try:
        ...
        for i in range(len(valid_time)):
            ...
            if os.path.exists(output_filepath):
                continue
            newValues = vals[i, ::-1, :].astype("float32")
            arr = xr.DataArray(
                    newValues,
                    dims=["lat", "lon"],
